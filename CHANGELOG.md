...
### CR-BP-106: AgencyAndOrganization Regulator-Tied Admission Tranche (5 Cells, Combined)

Combined single-PR admission of the 5 regulator-tied Business Processes at the AgencyAndOrganization x {Conceive, Design, Build, Operate, Improve} ECF coordinates, admitted under the CR-BP-105 escape-clause discovery outcome (10/10, ADMIT-CANONICAL recommendation) under the CR-BP-62 method. 5 PC replacements + 5 PG replacements + 5 new BPs + 5 BP READMEs = 20 new/modified files:

- ao-conceive: Conceive Licensed or Regulated Agent Capacity (FCA SMCR CF1..CF11; state healthcare licensure scopes; EU AI Act Art. 9 risk classification; US state bar admissions)
- ao-design: Design Regulated Role Architecture (FCA SMCR SYSC 25 Annex 1; FAA Part 121 subpart Y CRM; EU AI Act Art. 14 human-oversight roles; The Joint Commission credentialing-and-privileging)
- ao-build: Build Regulated Agent Pool (FCA Financial Services Register; NMLS / NAIC Producer Database; FAA airmen certification database; OFAC SDN screening per 31 CFR 501; Bank Secrecy Act per 31 USC 5311; FCRA per 15 USC 1681; ADA per 42 USC 12102)
- ao-operate: Operate Regulated Performance Oversight (FCA SMCR Conduct Rules (COCON); FINRA Rule 3110; FAA Part 117; state bar MCLE; state healthcare CME; NAIC CE; EU AI Act Art. 14 continuing-oversight)
- ao-improve: Improve Regulated Workforce Capability (FCA SMCR fit-and-proper annual assessment (SYSC 25 / FIT); state bar MCLE; state healthcare CME; NAIC CE; FAA Part 121 subpart Y recurrent training; EU AI Act Art. 14 ongoing training)

The 5 regulator-tied BPs complete the A&O regulator-tied chain (Conceive -> Design -> Build -> existing Activate (PR #122) -> Operate -> Improve -> existing Retire (PR #114)). A&O is the first domain with regulator-tied coverage across all 7 stages. Posture: 28 discovery records checked, 0 findings; IDM-001..007 CONFORMANT (3603 records); MECE-001..008 CONFORMANT; LCM-001..005 CONFORMANT; ACT-001..015 CONFORMANT (560 Activities, 145 BPs); BP-C1..C4 + BP-QUAL-001..012 CONFORMANT; S21 CONFORMANT; TASK-001..005 CONFORMANT (2800 Tasks); targeted pytest 167/167 passed; broad catalog validator sweep 244/244 passed; `open_change_requests` 120 -> 121. Zero TMForum references introduced (CR-BP-TMF-EMBARGO compliance continues). Lands after CR-BP-105 (#155) which established the discovery recommendations. L3 Activity and L4 Task decomposition lands in CR-BP-111 (deferred follow-on tranche).

### CR-BP-TMF-EMBARGO: TMForum reference embargo compliance strip

User directive 2026-09-22 (eaojnr, explicit religious mandate): NEVER reference TMForum (TM Forum, TMF, eTOM, Frameworx, GB921, GB925, SID, TAM, ODF, OSS/BSS) in any artifact of any org repo. Drops all 73 substantive TMForum citations across 24 merged-history files: 7 discovery records (CR-BP-63 baseline), 12 admitted entity files across EO/FA/PR/PR-Design/PV-Retire clusters, 3 PR.Operate research register files, 8 change-request documents (CR-BP-04 / -11 / -63 / -64 / -66 / -67 / -74 + README table), 2 tooling files (`tests/test_build_bp13a_tranche.py` whitelist, `tools/build_bp13a_tranche.py` evidence_links), 1 archive (`01_plan/archive/pr-bp-13a.md`). Substitution policy: drop, do not substitute. The PR.Design cluster gains an APQC PCF cite to maintain the `test_render_l2_process_uses_real_industry_evidence` regression guard (no TMForum cite reintroduced). Carrier: `change-requests/CR-BP-TMF-EMBARGO-tmforum-reference-strip.md`. Posture: 25 conformance gates CONFORMANT (10 blocking + 15 advisory); targeted pytest 39/39 passed; 0 new entity records; `CATALOG.yaml` unchanged; `reconciliation/conformance_report.yaml` runtime-path noise reverted. Next slice: A&O domain-deep sourcing tranche (Option C) gated on this landing.
...
Discovery-only slice under the CR-BP-62 escape-clause method, applying domain-deep sourcing (Option C of the programme doctrine) to the five AgencyAndOrganization ECF cells that previously had only the substrate-neutral general-case BP: Conceive, Design, Build, Operate, Improve. Five new escape records at `discovery/v1-alpha/agency-and-organization-{conceive,design,build,operate,improve}-escape.yaml`, each evaluating one regulator-tied candidate against BP-LIFE-001..015 with the canonical five-axis rubric. All five recommend ADMIT-CANONICAL (10/10):
...
The five regulator-tied candidates complete the upstream + downstream chain for A&O domain-deep sourcing, bridging the existing Activate licensed-workforce BP (PR #122 / CR-BP-81) and the existing Retire regulated-workforce-wind-down BP (PR #114 / CR-BP-73). Posture: 28 discovery records checked, 0 findings; 25 conformance gates CONFORMANT; `open_change_requests` 119 -> 120. Zero TMForum references introduced (CR-BP-TMF-EMBARGO compliance continues). Admission tranches (CR-BP-106..110 per-cell, each landing full PC + PG + BP stack) gate on this discovery slice. Lands after CR-BP-TMF-EMBARGO (#154) so the embargo remediation precedes the sourcing tranche.
