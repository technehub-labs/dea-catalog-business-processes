...
**Doctrine** (eaojnr 2026-09-19): L0 = ProcessContext (the canonical ECF cell matrix in `contexts/v1-alpha/`); L1 = ProcessGroup (the 48 canonical groups in `entities/v1-alpha/`); canonical cardinality is exactly 1 L1 per L0 (and 1 L0 per L1). The ProcessScope layer (`dea:scope-*`) introduced by CR-BP-95 is a schema-only artifact that has never been instantiated; CR-BP-100 declares it dormant because L0 IS the Process Context matrix.
...
**Lands against**: 48 PCs / 48 PGs / 139 BPs / 553 Activities / 0 ProcessScope records; 28 gates CONFORMANT; v0.4.0+10 commits on main.
...
| `CATALOG.yaml` | MOD | Regenerator bumps `open_change_requests` 111 → 112. |
...
2. **Tests second (exhaustive).** The test file audits the live catalog row-by-row:
   - `test_live_total_counts` :  48 PCs == 48 PGs.
   ...
   - `test_live_per_pc_audit` :  walks each of 48 PCs; fails with a per-PC diagnostic if cardinality slips.
   - `test_live_per_pg_audit` :  walks each of 48 PGs; fails with a per-PG diagnostic if `process_context` is malformed or unknown.
...
6. **Count assertions.** None changed. 48 PCs / 48 PGs invariant is the assertion; the slice does not introduce or remove records.
...
- Gate [24] L0↔L1 Cardinality (L0L1-CARD-001..003, CR-BP-100): **CONFORMANT, PASS** (48 PCs / 48 PGs / 0 ProcessScope records).
- Full suite: **29 gates, 0 blocking, 0 advisory failures**.
- `open_change_requests`: 111 → 112.

## 5. What this CR is NOT

- **NOT a population slice.** No new records land. The 48 PCs and 48 PGs are unchanged.
...
CR-BP-100 codifies the L0↔L1 1:1 cardinality invariant as an automated advisory gate (L0L1-CARD-001..003), wires it as gate [24] in the conformance pipeline, and declares the ProcessScope layer (CR-BP-95) dormant by design. The 48 canonical PCs and 48 canonical PGs are unchanged; the slice introduces no records. The 29-gate suite remains CONFORMANT (0 blocking, 0 advisory); `open_change_requests` 111 → 112. The documentation pipeline (EXT-02..06) may now open with L0↔L1 cardinality guaranteed by automated enforcement.