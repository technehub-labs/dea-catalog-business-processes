# Research register: `dea:process-operate-warehouse-and-inventory`

This directory holds research artifacts specific to
this L2 Business Process.

## Contents

- `l3-candidate-universe.yaml` (CR-BP-43, 2026-09-13):
  L3 (Activity) candidate universe decomposing
  `dea:process-operate-warehouse-and-inventory` into 4 candidates, each tested
  against the four L2 criteria (CR-BP-32 section 5),
  scored for cohesion (CR-BP-32 section 6, 0-9 scale),
  and assigned a disposition. All candidates landed as
  canonical Activity records under
  `entities/v1-alpha/dea:activity-*/`.

## Provenance

Established by CR-BP-43 on 2026-09-13 as part of the
EnablementAndOperations / Operate L3 tranche (pattern
proven by the CR-BP-42 pilot).

## Governing CRs

- **CR-BP-21d.1**: initial admission of the parent L2
  Business Process as part of the
  EnablementAndOperations completion tranche.
- **CR-BP-32**: Activity Model (definition, cohesion,
  composition, boundary; ACT-001..010).
- **CR-BP-42**: L3 Activity pilot deposition (pattern
  origin, dea:process-operate-quality-control).
- **CR-BP-43**: L3 EO/Operate tranche (this directory's
  first artifact).
