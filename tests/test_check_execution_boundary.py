...
def test_cli_live_run_returns_conformant():
    """Live run: 672 records (161 pre-CR-BP-42 + 501 Activity records from CR-BP-42..57 + 4 CR-BP-64 + 2 CR-BP-65 + 2 CR-BP-66 + 2 CR-BP-67 admission records), 0 opted-in, 0 findings."""
    result = _run([])
    assert result.returncode == 0, result.stdout + result.stderr
    assert "Execution Boundary (CR-BP-33; EXE-001..010): CONFORMANT" in result.stdout
    assert "Records checked:   672" in result.stdout
