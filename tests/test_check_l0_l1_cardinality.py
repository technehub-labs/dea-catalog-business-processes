...
CR-BP-100 (eaojnr 2026-09-19 doctrine). L0 = ProcessContext (dea:pc-*);
L1 = ProcessGroup (dea:group-*). Canonical cardinality is exactly 1 L1
per L0 (and exactly 1 L0 per L1).

These tests run exhaustively against the LIVE catalog to prove every
PC and every PG satisfy the 1:1 invariant. The live catalog has 48 PCs
and 48 PGs as of 2026-09-19; this suite verifies that count + the
pairwise mapping + absence of any ProcessScope records.

Coverage:
  - Live catalog aggregate: 48 PCs, 48 PGs, every PC has exactly 1 PG,
    every PG resolves to exactly 1 known PC.
  - Live catalog per-PC audit: each of 48 PC ids appears exactly once
    in the by-pc map with exactly 1 group.
  - Live catalog per-PG audit: each of 48 PG ids has a process_context
...
def test_live_total_counts(live):
    """48 PCs, 48 PGs as of 2026-09-19."""
    pc_ids, groups, scopes = live
    # 48 confirmed: 7 domains x 7 stages = 49 max; 48 currently landed.
    assert len(pc_ids) == 48, (
        f"Expected 48 canonical Process Contexts, found {len(pc_ids)}. "
        f"If a new PC was admitted, update this assertion AND the live "
        f"catalog reference in CR-BP-100."
    )
    assert len(groups) == 48, (
        f"Expected 48 canonical Process Groups, found {len(groups)}. "
